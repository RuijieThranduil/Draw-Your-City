...
[CustomEditor(typeof(GPSUtil))]
public class GPSUtilEditor : Editor
{
    public override void OnInspectorGUI()
    {
        base.OnInspectorGUI();
        
        GPSUtil gps = (GPSUtil)target;
        if (GUILayout.Button("GCJ02_to_WGS84"))
        {
            GPSUtil.GCJ02_to_WGS84(gps.Latitude, gps.Longitude, out double wgsLat, out double wgsLng);
            Debug.Log($"GCJ02_to_WGS84: {wgsLat}, {wgsLng}");
        }

        if (GUILayout.Button("WGS84_to_GCJ02"))
        {
            GPSUtil.WGS84_to_GCJ02(gps.Latitude, gps.Longitude, out double gcjLat, out double gcjLng);
            Debug.Log($"WGS84_to_GCJ02: {gcjLat}, {gcjLng}");
        }
    }
}
#endif